$(document).ready(function () {
    var per=0;
    var filler=$('#filler'); //0:sessionPaused 1:running 2:break running 3:break paused
    var state=0;
    var timeLapsed; //for window setTimeInterval
    var timer;
    ...
    $('#breakplus,#sessionplus').on('click', function () {
        ...
        if ($(this).attr('id')=="sessionplus") {
            if (state==3) return;
            per=0;
            timeLapsed=0 color=colorPool[Math.floor((Math.random() * 6))];
        } //reset per and timelapsed if breaker time changed
    ...
    $('.clockcontainer').on('click', function () {
        ...
        if (state==0) {
            ...
            timer=setInterval(function () {
                sessionTimer(factor, color)
            }
            , 1000);
        }
        else if (state==1) {
            //pause the timer
            clearInterval(timer); //change to session paused
            state=0;
        }
        else if (state==2) {
            clearInterval(timer); //change to  break paused
            state=3;
        }
        else {
    ...
    var transferFromSecondsToTime=function (seconds) {
        ...
        m=Math.floor(m / 60);
        m=m < 10 ? '0' + m: m;
        var s=seconds % 3600 % 60 s=s < 10 ? '0' + s: s;
    ...
    var sessionTimer=function (factor, c) {
        if (per > 100) {
            ...
            , 1000);
            $('#sessiontext').text("Break!") return;
        }
        per +=(1 / (60 * factor)) * 100;
        var height=per + '%';
        $(filler).css("background", "linear-gradient(to top, " + color + " " + height + ",#536359 " + height + ")");
        var displayTime=transferFromSecondsToTime(--timeLapsed) $(timetext).text(displayTime);
    } //parameter is set to c because dont want to mix up the local and global variable
    var breakTimer=function (factor, c) {
        if (per > 100) {
            ...
            , 1000);
            $('#sessiontext').text("Session") return;
        }
        per +=(1 / (60 * factor)) * 100;
        var height=per + '%';
        $(filler).css("background", "linear-gradient(to top, " + color + " " + height + ",#536359 " + height + ")");
